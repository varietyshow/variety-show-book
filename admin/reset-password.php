...
if (isset($_GET['token'])) {
    ...
    $stmt = $conn->prepare("SELECT admin_id FROM admin_account WHERE reset_token = ? AND reset_expiry > NOW()");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $validToken) {
    ...
    if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
        ...
        $updateStmt = $conn->prepare("UPDATE admin_account SET password = ?, reset_token = NULL, reset_expiry = NULL WHERE reset_token = ?");
        ...
        if ($updateStmt->execute()) {
            ...
            header("refresh:3;url=admin-loginpage.php");
        } else {
...
<body>
    <div class="container">
        ...
        <div class="back-link">
            <a href="admin-loginpage.php">Back to Login</a>
        </div>
    </div>
    ...
    <script>
        ...
        passwordInput.addEventListener('input', function() {
            ...
            // Add this at the end of the function to update confirm password field
