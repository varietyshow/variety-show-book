...
function retrievePayment($payment_id) {
    try {
        ...
        error_log("Error in retrievePayment: " . $e->getMessage());
        return ['error' => $e->getMessage()];
    }
}
...
function makePayMongoRequest($endpoint, $data = null, $method = 'POST') {
    try {
        ...
        error_log("Error in makePayMongoRequest: " . $e->getMessage());
        return ['error' => $e->getMessage(), 'http_code' => 0];
    }
}
...
function logPayMongoActivity($message, $data = []) {
    try {
        ...
        error_log("Error in logPayMongoActivity: " . $e->getMessage());
    }
}
