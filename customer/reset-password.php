...
if (isset($_GET['token'])) {
    ...
    $stmt = $conn->prepare("SELECT customer_id FROM customer_account WHERE reset_token = ? AND reset_expiry > NOW()");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && $validToken) {
    ...
    if(!$uppercase || !$lowercase || !$number || !$specialChars || strlen($password) < 8) {
        ...
        $updateStmt = $conn->prepare("UPDATE customer_account SET password = ?, reset_token = NULL, reset_expiry = NULL WHERE reset_token = ?");
        ...
        if ($updateStmt->execute()) {
            ...
            header("refresh:3;url=customer-loginpage.php");
        } else {
...
<body>
    <div class="container">
        ...
        <div class="back-link">
            <a href="customer-loginpage.php">Back to Login</a>
        </div>
    </div>
